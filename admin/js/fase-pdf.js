...
(function () {
  ...
  function giornoLabel(key, day) {
    var map = { ab: "Lunedì", ac: "Mercoledì", cb: "Sabato" };
    return (day && day.giorno) || map[key] || "";
  }
  ...
  function render(macro, fase, root, blocco) {
    ...
    if (titleEl) {
      titleEl.textContent = "PDF riassunto AB–CB · " + fase.nome.replace(/^Fase \d+ · /, "");
    }
    ...
    if (dl) {
      var files = {
        "ipertrofia-accumulo": {
          href: u("/admin/prototipi/periodizzazione/fase/scheda-fase-1-ipertrofia-accumulo.pdf"),
    ...
    head.innerHTML =
      ...
      "<span><strong>RIR:</strong> " + (fase.rir || blocco.rir || "—") + "</span>" +
    ...
    var introText = (fase.guida || fase.obiettivo || blocco.guida || "").slice(0, 200);
    oss.innerHTML =
      "<div class=\"scheda-a4__osservazioni-label\">Osservazioni / note</div>" +
      "<p class=\"scheda-a4__intro-text\">" +
      (introText ? introText + " — " : "") +
      "kg a penna · * = progressione · gambe solo mantenimento</p>";
    ...
    SESSION_KEYS.forEach(function (key) {
      ...
      var subParts = [giornoLabel(key, day), day.quotaVolume].filter(Boolean);
      if (subParts.length) {
        h2.appendChild(document.createElement("br"));
        h2.appendChild(el("span", { className: "scheda-a4__quad-sub", text: subParts.join(" · ") }));
      }
      quad.appendChild(h2);
    ...
    article.appendChild(el("footer", {
      ...
      text: fase.nome + " · Nicola · * = progressione · kg da compilare · uso palestra"
    }));
  ...
  function init() {
    ...
    if (!faseId) {
      root.innerHTML = "<p>Parametro <code>fase</code> mancante. <a href=\"" + u("/") + "\">Torna al ciclo</a></p>";
      return;
    }
